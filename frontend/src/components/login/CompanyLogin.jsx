    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(
                "http://localhost:5000/api/v1/company/login", // Changed API endpoint for admin login
                { email, password },
                {
                    headers: {
            ...
            const { accessToken, refreshToken, company } = data; // Access data from the response
            ...
            <div>
                <form className={css.form} onSubmit={handleSubmit}>
                    <div className={css.leftcomp}>
                        <span className={css.signup}>Company Log In</span> {/* Updated title */}
                        <input
                            type="email"
